...
export class KVMerkleTree {
  ...
  public getMerklePathFromKey(key: string): MerklePath {
    if (this.pointers.isEmpty()) throw new Error("Your tree is generate from leaves. No key / values available. Please use getMerklePathFromLeaf instead.");
  ...
  public getValue(key: string): BigNumber {
    if (this.pointers.isEmpty()) throw new Error("Your tree is generate from leaves. No key / values available.");
  ...
  static fromLeaves(leaves: string[], hashFunction: HashFunction, forceHeight: number | null = null, hashLeaves: boolean = false): KVMerkleTree {
