...
describe("Merkle tree lib", () => {
  ...
  describe("Merkle tree from data", () => {
    ...
    it("Should get the root of the Merkle tree", async () => {
      merkleTreeRoot = BigNumber.from(
        "4047207450062892694258929318783786787130166100928326367429339529679014639025"
      );
      expect(accountMerkleTree.getRoot().toHexString()).to.equals(merkleTreeRoot.toHexString());
    });

    it("Should get a value from a key", async () => {
      expect(accountMerkleTree.getValue(account).toHexString()).to.equals(
    ...
    it("Should get the the leaf with the key", async () => {
      expect(accountMerkleTree.getLeaf(account).toHexString()).to.equals(
  ...
  describe("Merkle tree from leaves", () => {
    beforeAll(async () => {
      ...
      leaves = ["123", "457", "124", "458", "125", leaf, "126", "456"];

      fromLeavesMerkleTree = KVMerkleTree.fromLeaves(leaves, poseidon);
    });

    it("Should get the root of the Merkle tree", async () => {
      expect(fromLeavesMerkleTree.getRoot().toHexString()).to.equals(
        "0x0aacb0fd8830bb75d9a5ef5b57fd658e60f06c8fa427cf9ab2eec06184bcc429"
      );
    });
    ...
    it("Should throw when trying to get a value from a key", async () => {
      try {
        ...
        expect(e.message).to.have.string(
          "Your tree is generate from leaves. No key / values available."
        );
      }
    });
  ...
  describe("Merkle tree optimized formatting", () => {
    ...
    it("Should go from json to treeOptimizedFormatV1 and back to json", async () => {
      ...
      const fromTreeFormatV1MerkleTree =
        KVMerkleTree.fromTreeOptimizedFormatV1(treeOptimizedFormatV1);
    ...
    it("Merkle tree with 0 as an accountIdentifier should work", async () => {
      const dataMerkleTreeUnbalanced2 = {
        "0x0": 0,
        "0x0085560b24769dac4ed057f1b2ae40746aa9aab6": 1,
        "0x0294350d7cf2c145446358b6461c1610927b3a87": 1,
...
function generateIncrementalArray(n: number): number[] {
  let array: number[] = [];
  for (let i = 0; i < n; i++) {
    array.push(i);
  }
